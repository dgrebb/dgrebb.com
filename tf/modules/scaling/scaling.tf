resource "aws_alb" "cms" {
  name               = var.dashed_cmsdomain #load balancer name
...
resource "aws_lb_listener" "https_listener" {
  load_balancer_arn = aws_alb.cms.arn #  load balancer
  port              = 443
  protocol          = "HTTPS"
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.cms.arn # target group
  }
